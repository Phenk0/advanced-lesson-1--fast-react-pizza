...
function Button({ isDisabled, children, to, type, onClick }) {
  const base =
  ...
  const styles = {
    ...
    secondary:
      "inline-block rounded-full border-2 border-stone-300 px-4 font-semibold uppercase tracking-wide text-stone-400 transition-colors duration-300 py-2.5 hover:bg-stone-300 hover:text-stone-800 focus:bg-stone-300 focus:text-stone-800 focus:outline-none focus:ring focus:ring-stone-200 focus:ring-offset-2 disabled:cursor-not-allowed md:py-3.5 md:px-6",
  };
  if (to)
    return (
